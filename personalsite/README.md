...
The homepage reads published rows only, cached and revalidated at most every 60 seconds; a save can take up to that long to appear there. Full entries appear at `/projects/[slug]` and `/thoughts/[slug]`, which stay fully dynamic so publish/unpublish is reflected immediately. Only detail pages download Markdown; lists use table columns. Public reads use a separate anonymous client even while the owner is signed in. Markdown is rendered with GitHub-flavored Markdown support; raw HTML is not rendered.
